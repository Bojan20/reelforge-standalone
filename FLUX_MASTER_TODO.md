...
| 5.1.3 | `generate_stage_audio(stage, style, duration)` FFI | `sam_ffi.rs` | ✅ Sprint 18 — `crates/rf-bridge/src/generative_ffi.rs` (220 LOC) sa JSON-in / PCM-out C ABI: `generative_generate(*const c_char) → GenerativeFfiBuffer` + `generative_free_buffer` + `generative_free_string`. `GenerativeFfiBuffer` repr(C) sa `pcm_ptr: *mut f32`, `pcm_len: usize`, `sample_rate_hz`, `channels`, `latency_ms`, `metadata_json`, `error_json` — direktan PCM transfer izbegava 30 MB base64 round-trip kroz JSON. `Box::into_raw` → Dart owns; matching `Box::from_raw` u free. 8/8 FFI testova zelena (null/invalid/valid request, byte-identical determinism cross-FFI, arc round-trip, out-of-range rejection, idempotent free). Dart strana: `flutter_ui/lib/services/generative_audio_service.dart` (~280 LOC) singleton sa lazy `DynamicLibrary.open('librf_bridge.dylib')`, repr(C) `_GenerativeFfiBuffer` struct mapper, async `generate(GenerationRequest) → GenerationResult` API. Dart modeli: `GenerationRequest`, `GenerationStyle`, `SlotStageHint` (12 variants, snake_case wire), `EmotionalArc`, `GenerationResult`, `GenerationMetadata`, `GenerationException`. Auto-free try/finally. `serde(default)` na `seed`/`sample_rate_hz`/`style` polja → omitted polja u JSON-u rade. 11/11 Dart unit testova zelena (JSON shape, enum wire names ratchet, metadata parsing, exception/frameCount semantics). flutter analyze 0 issues, cargo check workspace clean. **Stage 5.1.4 UI (GEN sub-tab) sad UNBLOCKED.** |
| 5.1.4 | "GEN" sub-tab u MUSIC ili MONITOR super-tab | UI | ✅ Sprint 18 — `flutter_ui/lib/widgets/slot_lab/lower_zone/slotlab_music_gen_panel.dart` (~560 LOC) `SlotLabMusicGenPanel` mounted u MUSIC super-tab kao 8. sub-tab (`SlotLabMusicSubTab.gen`, shortcut `I`). Layout reflow ≥720 px = dva stupca (`controls | output`), ispod = stacked SingleChildScrollView. Controls: prompt `TextField` (2 reda), duration slider 1.0–30.0 s (0.5 s koraci), stage hint dropdown sa svih 12 `SlotStageHint` opcija + "unspecified" (null prosleđuje `style.stage_hint` izostavljeno), seed `TextField` (optional, parsuje int ili šalje null), embedded `EmotionalArcEditor` (height 140 px, presets ON) → `onChanged` čuva `EmotionalArc` u state-u. `GENERATE` button → `GenerativeAudioService.instance.generate(GenerationRequest)` (FFI seam: `GenerateFn typedef` injectable za testove). Output card: PCM sparkline (downsampled min/max bars u brandGold), provenance kv-list (`backend_id`/`model_id`/`seed`/`duration_seconds`/`latency_ms`/`generated_at_utc`) + `wall-clock` Stopwatch za sanity-check (Mock < 5 ms, real ONNX ~8 s na M3). Validacija: prazan prompt short-circuit-uje sa "Prompt is required" banner-om (no FFI call). `GenerationException` se hvata i prikazuje u crvenom banner-u — output zadržava placeholder ("No clip yet"). Audio playback OUT-OF-SCOPE — `Float32List` se vraća, routing u slot audio graph dolazi sa 5.1.6/5.1.7 (style transfer + variation generation) kad je slot context known. **7/7 widget testova zelena** (render smoke, request forwarding, success provenance, error banner, empty-prompt validation, seed parsing, narrow layout reflow). flutter analyze 0 issues. `SlotLabMusicSubTabX` extension proširena (label="GEN", tooltip="local generative audio (prompt + arc → PCM)"). **FAZA 5 generativni layer sad ima end-to-end UI surface — Rust crate → FFI → Dart service → user-facing panel.** |
| 5.1.5 | Emotional arc timeline input (tension → excitement → euphoria) | UI | ✅ Sprint 18 — `flutter_ui/lib/widgets/generative/emotional_arc_editor.dart` (~530 LOC) pure-Dart `CustomPaint` widget. `EmotionalArcEditor(initial, onChanged, height, showPresets)` API. Gestures: tap empty area → insert point; drag point → move (interior t clamped between neighbours, endpoints t-pinned to 0/1); double-tap point → delete (endpoints protected). 8 preset arc-ova (`flat`, `crescendo`, `decrescendo`, `spike`, `dip`, `tenseBuildup`, `euphoricPayoff`, `doublePeak`) — slot-domain naming (model razume "crescendo" bolje od "ramp_up"). Stateless `EmotionalArcOps` (normalize / insertPoint / movePoint / deletePoint / hitTest / sample) za pure-data testabilnost bez `WidgetTester`. Visual: glass background + 5×5 grid, brandGold gradient fill + brandGoldBright stroke, brandGoldDeep endpoint dots vs brandGold interior dots, accentCyan halo na active/hover. Footer Wrap reflow za narrow containere. Output direktno `EmotionalArc` (isti tip koji `GenerativeAudioService.generate` šalje Rust strani). 28/28 testova zelena uključujući Rust parity test za `sample()` math (isti test case kao `rf-generative::arc_sample_clamps_and_interpolates`). flutter analyze 0 issues. **5.1.4 GEN sub-tab sad može da konzumira ovaj widget direktno.** |
| 5.1.6 | Style transfer iz reference slota | Model | 🔴 OPEN |
| 5.1.7 | Variation generation (5 alternate BIG_WIN stings) | 1 klik | ✅ Sprint 18 — `GenerativeAudioService.generateVariations(req, count)` (Dart, ~70 LOC) fan-outs single request u N alternatives. Count clamps `[1, 10]`. Base seed = `request.seed` kad je pinned, inače FNV-1a 64 nad `(prompt + duration + stage_hint + arc + tags)` → stabilan auto-seed (isti brief → ista varijaciona serija kroz restart). Step = **7919 (prime)** tako da susedne varijacije ne padaju u susedne SplitMix64 buckets sa sličnim spektrom. Atomic semantika — bilo koja varijacija fail → ceo poziv throws (no partial lists; UI ne mora da pita "koja varijacija fali"). Sekvencijalno (mock je sub-ms; budući ONNX backend ide na worker isolate isto). `GenerationRequest.withSeed(int)` helper za immutable seed swap. UI: GEN panel ima drugo dugme **× 5** (outlined, brandGoldBright) pored GENERATE. Output card prepend-uje `_variationStrip` (horizontal `ListView` sa 5× `_VariationCard` — 96 px tile sa index badge `#1..#5`, seed monospace, mini sparkline). Tap card → promotuje varijaciju u glavni waveform/provenance view (`_selectedVariationIndex` + `_last` sinhronizuju). Plain GENERATE posle variations brisi strip (semantika: "tražio sam jedan, ne pet"). Empty prompt block-uje variations call isto kao single GENERATE. **5/5 widget testova zelena** (strip renders 5 cards, tap promotes seed, GENERATE clears strip, error surface, empty-prompt block) + **9/9 service testova** (`withSeed` field preservation, JSON shape, seed stepping, clamp, uniqueness, atomic-failure contract, request round-trip, Float32List independence). flutter analyze 0 issues. **End-to-end "1 klik za 5 alternates" path live.** |
| 5.1.8 | Auto-compliance validator na generisan audio | `rf-slot-builder` | ✅ Sprint 18 — `crates/rf-generative/src/compliance.rs` (~440 LOC) pure-Rust validator koji svaki generisani clip dobija inline u response-u. `ComplianceLevel { Pass, Warn, Fail }` (derive `PartialOrd`/`Ord` — `Fail > Warn > Pass` koristi se za aggregate). `ComplianceFinding { id, level, message, value }` — `id` je stable kebab-case (`clean`/`empty-pcm`/`non-finite-samples`/`clipping`/`all-silence`/`mostly-silent`/`peak-too-hot`/`peak-too-low`/`dc-offset`/`sample-rate-mismatch`/`frame-count-mismatch`/`very-short-clip`/`no-report`) tako da UI / audit pipeline filtriraju bez string match-a. `ComplianceReport { level, findings, peak_dbfs, rms_dbfs, dc_offset, clip_count, nan_count, silence_ratio, duration_seconds }`. `compute_compliance(&response, &request)` pure fn — single-pass O(N) sample scan (NaN/Inf, clip threshold 0.999, silence 1e-4) + cross-check vs request (sample_rate, frame count ±2 tolerance, duration ≥ 0.1s). Hard fails: empty PCM / NaN / clipping / all-silence / sample-rate mismatch / frame-count mismatch / bad channel count. Warns: peak > -1 dBFS, peak < -30 dBFS, |DC| > 0.01, silence > 90%, clip < 100ms. **Determinism contract**: isti `(response, request)` → byte-identical report (variation panel se oslanja). `GenerationResponse.compliance` polje popunjeno automatski u `MockBackend::generate` posle PCM gen-a — sledeći ONNX backend će ga isto popuniti via isti `compute_compliance` poziv. **11/11 Rust unit testova zelena** (clean pass, NaN fail, clipping fail, all-silence fail, DC warn, peak-too-hot warn, empty short-circuit, frame-count mismatch, level ordering, determinism, serde round-trip, dBFS math). FFI: `GenerativeMetadata.compliance: &ComplianceReport` inline u metadata_json — Dart parsira jedan JSON object, nema extra FFI call. **1/1 novi FFI test** validira shape (`level` ∈ pass/warn/fail, sve numeric polja prisutna). Dart: `ComplianceLevel.parse` permissive (unknown → warn, ne silent pass), `ComplianceFinding.fromJson` + `ComplianceReport.fromJson` (sa `_readDouble` koji handluje `-Infinity`/`NaN` string forme jer serde_json emit-uje `-Infinity` bez navodnika). `GenerationMetadata.compliance` opciono polje (const `_legacyUnknown` default → backward compatible). `ComplianceReport.unknown()` factory + `worstFindings` filter. **12/12 Dart service testova zelena**. UI: GEN panel provenance card dobija badge u header-u (PASS zelena / WARN amber / FAIL crvena sa Material ikonom `verified` / `warning_amber` / `gpp_bad`) + details strip ispod sa 2×2 metric grid (peak/rms dBFS, clips count, silence %) i top-5 finding messages sa color-coded bullet-ima. **3/3 widget testova** (PASS/WARN/FAIL badge rendering). flutter analyze 0 issues, cargo clippy -p rf-generative clean. **FAZA 5.1 ON-DEVICE generativni layer zatvoren — svaki clip ima provenance + auto-compliance manifest pre nego što izađe iz backend-a.** |
