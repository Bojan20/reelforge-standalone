...
Dva povezana stereo processing feature-a: StereoImager fix (postoji ali disconnected) + Haas Delay (novi) + MultibandStereoImager (iZotope Ozone Imager nivo).

**Specifikacija:** `.claude/architecture/HAAS_DELAY_AND_STEREO_IMAGER.md`
**Target Quality:** iZotope Ozone Imager ili bolji — multiband, vectorscope, stereoize, correlation

### Problem

`STEREO_IMAGERS` HashMap u `ffi.rs:9557` ima 15+ FFI funkcija, ali `playback.rs` ih **NIKADA ne poziva** — identičan bug pattern kao ranije popravljeni `DYNAMICS_COMPRESSORS`.
...
**Signal Flow (SSL kanonski):**
```
Input → Pre-Fader Inserts → Fader → Pan → ★ STEREO IMAGER → Post-Fader Inserts (incl. Haas) → Sends → Bus
```

**iZotope Parity:**
| Ozone Feature | FluxForge | Status |
|---|---|---|
| 4-band multiband width | `MultibandStereoImager` | Phase 4 |
| Stereoize (mono→stereo) | `Stereoize` allpass decorrelation | Phase 4 |
| Polar/Lissajous vectorscope | `VectorscopeWidget` (3 modes) | Phase 5 |
| Correlation meter | Already in rf-dsp | ✅ EXISTS |
| Per-band width slider | `BandImager` struct | Phase 4 |

**FluxForge BEYOND Ozone:**
- Channel strip integration (width on every track, not just plugin)
- Dedicated Haas Delay mode (Ozone has none)
- Stereo rotation (Ozone has none)
- Independent M/S gain (mid/side separate control)

---

*Last Updated: 2026-02-22 — Stereo Imager + Haas Delay + MultibandStereoImager (iZotope Ozone level) PLANNED. Total: 467 tasks (422 complete + 45 planned), 4,532 tests, 0 errors.*
