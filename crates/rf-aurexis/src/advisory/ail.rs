...
impl AilStatus {
    pub fn from_score(score: f64) -> Self {
        if score >= 90.0 { Self::Excellent }
        else if score >= 75.0 { Self::Good }
        else if score >= 60.0 { Self::Fair }
        else if score >= 40.0 { Self::Poor }
        else { Self::Critical }
    }
...
impl AuthoringIntelligence {
    ...
    pub fn analyze(&mut self, pbse_result: Option<&PbseResult>) -> AilReport {
        ...
        maybe_recommend(&mut recommendations, &hook_analysis,
        ...
        maybe_recommend(&mut recommendations, &volatility_analysis,
        ...
        maybe_recommend(&mut recommendations, &cascade_analysis,
        ...
        maybe_recommend(&mut recommendations, &overlap_analysis,
        ...
        maybe_recommend(&mut recommendations, &emotional_analysis,
        ...
        maybe_recommend(&mut recommendations, &energy_analysis,
        ...
        maybe_recommend(&mut recommendations, &voice_analysis,
        ...
        maybe_recommend(&mut recommendations, &spectral_analysis,
        ...
        maybe_recommend(&mut recommendations, &fatigue_analysis,
        ...
        maybe_recommend(&mut recommendations, &drift_analysis,
        ...
        recommendations.sort_by(|a, b| b.impact_score.partial_cmp(&a.impact_score).unwrap_or(std::cmp::Ordering::Equal));
        for (i, rec) in recommendations.iter_mut().enumerate() {
            rec.rank = (i + 1) as u32;
        }

        // Compute overall AIL Score: 100 × (1.0 - avg_risk)
        let avg_risk = domain_analyses.iter().map(|d| d.risk).sum::<f64>() / domain_analyses.len() as f64;
        ...
        let critical_count = recommendations.iter().filter(|r| r.level == RecommendationLevel::Critical).count() as u32;
        let warning_count = recommendations.iter().filter(|r| r.level == RecommendationLevel::Warning).count() as u32;
        let info_count = recommendations.iter().filter(|r| r.level == RecommendationLevel::Info).count() as u32;
...
fn level_from_risk(risk: f64) -> RecommendationLevel {
    if risk >= 0.7 { RecommendationLevel::Critical }
    else if risk >= 0.4 { RecommendationLevel::Warning }
    else { RecommendationLevel::Info }
}
...
fn analyze_hook_frequency(steps: &[SimulationStep]) -> DomainAnalysis {
    ...
    let burst_ratio = if total > 0 { burst_count as f64 / total as f64 } else { 0.0 };
    ...
    DomainAnalysis {
        ...
        score, risk,
        details: format!("total={} bursts={} max_consecutive={}", total, burst_count, max_consecutive),
    }
}

/// Domain 1: Volatility Pattern Detection
fn analyze_volatility_pattern(outputs: &[DeterministicParameterMap]) -> (DomainAnalysis, VolatilityAlignment) {
    if outputs.is_empty() {
        let analysis = DomainAnalysis { domain: AilDomain::VolatilityPattern, score: 100.0, risk: 0.0, details: "No data".into() };
        let align = VolatilityAlignment { alignment_score: 100.0, energy_variance: 0.0, escalation_range: 0.0 };
...
fn analyze_cascade_density(steps: &[SimulationStep]) -> DomainAnalysis {
    ...
    DomainAnalysis {
        ...
        score, risk,
        details: format!("cascades={} max_depth={} cascade_pct={:.1}%", cascade_count, max_depth, cascade_pct * 100.0),
    }
}
...
fn analyze_feature_overlap(steps: &[SimulationStep]) -> DomainAnalysis {
    ...
    let risk = (concurrent_highs as f64 * 0.05 + if max_concurrent > 10 { 0.3 } else { 0.0 }).clamp(0.0, 1.0);
    let score = ((1.0 - risk) * 100.0).clamp(0.0, 100.0);

    DomainAnalysis {
        domain: AilDomain::FeatureOverlap,
        score, risk,
        details: format!("overlap_events={} max_concurrent={}", concurrent_highs, max_concurrent),
    }
}
...
fn analyze_emotional_curve(outputs: &[DeterministicParameterMap]) -> DomainAnalysis {
    if outputs.len() < 2 {
        return DomainAnalysis { domain: AilDomain::EmotionalCurve, score: 100.0, risk: 0.0, details: "Insufficient data".into() };
    }
    ...
    let risk = if jump_count > 20 { 0.7 }
        else if jump_count > 10 { 0.4 }
        else if max_delta > 50.0 { 0.3 }
        else { (1.0 - smoothness).clamp(0.0, 0.3) };
    ...
    DomainAnalysis {
        ...
        score, risk,
        details: format!("avg_delta={:.3} max_delta={:.1} jumps={} smoothness={:.3}", avg_delta, max_delta, jump_count, smoothness),
    }
}
...
fn analyze_energy_distribution(outputs: &[DeterministicParameterMap]) -> DomainAnalysis {
    if outputs.is_empty() {
        return DomainAnalysis { domain: AilDomain::EnergyDistribution, score: 100.0, risk: 0.0, details: "No data".into() };
    }
    ...
    let avg_reverb = outputs.iter().map(|o| (o.reverb_send_bias + 1.0) / 2.0).sum::<f64>() / n;

    let domains = [avg_energy, avg_transient / 2.0, avg_width / 2.0, avg_harmonic / 2.0, avg_reverb];
    let domain_mean = domains.iter().sum::<f64>() / domains.len() as f64;
    let domain_variance = domains.iter().map(|d| (d - domain_mean).powi(2)).sum::<f64>() / domains.len() as f64;
    ...
    DomainAnalysis {
        ...
        score, risk,
        details: format!("imbalance={:.4} E={:.2} T={:.2} W={:.2} H={:.2} R={:.2}",
            imbalance, avg_energy, avg_transient, avg_width, avg_harmonic, avg_reverb),
    }
}

/// Domain 6: Voice Utilization
fn analyze_voice_utilization(outputs: &[DeterministicParameterMap]) -> (DomainAnalysis, VoiceEfficiency) {
    let budget = 48u32;

    let voice_estimates: Vec<u32> = outputs.iter().map(|o| {
        let base = o.center_occupancy;
        let from_energy = (o.energy_density * 8.0) as u32;
        base + from_energy
    }).collect();

    let avg_voices = if voice_estimates.is_empty() { 0.0 }
        else { voice_estimates.iter().sum::<u32>() as f64 / voice_estimates.len() as f64 };
    let peak_voices = voice_estimates.iter().cloned().max().unwrap_or(0);

    let utilization = if budget > 0 { avg_voices / budget as f64 } else { 0.0 };
    let efficiency = utilization.clamp(0.0, 1.0);

    let risk: f64 = if utilization > 0.95 { 0.7 }
        else if utilization > 0.9 { 0.5 }
        else if utilization < 0.2 { 0.3 }
        else { 0.1 };
    ...
    let analysis = DomainAnalysis {
        ...
        score, risk,
        details: format!("avg={:.1} peak={} budget={} util={:.1}%", avg_voices, peak_voices, budget, utilization * 100.0),
    };

    let eff = VoiceEfficiency {
        avg_voices, peak_voices, budget_cap: budget,
...
fn analyze_spectral_overlap(outputs: &[DeterministicParameterMap]) -> (DomainAnalysis, SpectralClarityAnalysis) {
    if outputs.is_empty() {
        let analysis = DomainAnalysis { domain: AilDomain::SpectralOverlap, score: 100.0, risk: 0.0, details: "No data".into() };
        let clarity = SpectralClarityAnalysis { sci_advanced: 0.0, sci_limit: 0.85, clarity_score: 100.0, overlap_count: 0 };
        return (analysis, clarity);
    }

    // Estimate SCI from harmonic excitation × energy density × occupancy
    let sci_estimates: Vec<f64> = outputs.iter().map(|o| {
    ...
    let risk: f64 = if avg_sci > 0.85 { 0.8 }
        else if avg_sci > 0.75 { 0.5 }
        else if avg_sci > 0.6 { 0.3 }
        else { 0.1 };
    ...
    let analysis = DomainAnalysis {
        domain: AilDomain::SpectralOverlap, score, risk,
        details: format!("avg_sci={:.4} max_sci={:.4} overlaps={}", avg_sci, max_sci, overlap_count),
    };

    let clarity = SpectralClarityAnalysis { sci_advanced: avg_sci, sci_limit: 0.85, clarity_score, overlap_count };

    (analysis, clarity)
}

/// Domain 8: Fatigue Projection
fn analyze_fatigue_projection(outputs: &[DeterministicParameterMap]) -> (DomainAnalysis, FatigueAnalysis) {
    if outputs.is_empty() {
        let analysis = DomainAnalysis { domain: AilDomain::FatigueProjection, score: 100.0, risk: 0.0, details: "No data".into() };
        let fatigue = FatigueAnalysis { fatigue_score: 0.0, peak_frequency: 0.0, harmonic_density: 0.0, temporal_density: 0.0, recovery_factor: 1.0, risk_level: "LOW" };
    ...
    let risk_level = if fatigue_score > 70.0 { "CRITICAL" }
        else if fatigue_score > 50.0 { "HIGH" }
        else if fatigue_score > 30.0 { "MODERATE" }
        else { "LOW" };
    ...
    let analysis = DomainAnalysis {
        domain: AilDomain::FatigueProjection, score, risk,
        details: format!("fatigue_idx={:.4} PF={:.3} HD={:.3} TD={:.3} RF={:.3}",
            fatigue_index, peak_frequency, harmonic_density, temporal_density, recovery_factor),
    };

    let fatigue = FatigueAnalysis { fatigue_score, peak_frequency, harmonic_density, temporal_density, recovery_factor, risk_level };
...
fn analyze_session_drift(outputs: &[DeterministicParameterMap]) -> DomainAnalysis {
    if outputs.len() < 20 {
        return DomainAnalysis { domain: AilDomain::SessionDrift, score: 100.0, risk: 0.0, details: "Insufficient data".into() };
    }

    let quarter = outputs.len() / 4;
    let q1_avg = outputs[..quarter].iter().map(|o| o.energy_density).sum::<f64>() / quarter as f64;
    let q4_avg = outputs[3*quarter..].iter().map(|o| o.energy_density).sum::<f64>() / quarter as f64;

    let drift = q4_avg - q1_avg;

    let esc_values: Vec<f64> = outputs.iter().map(|o| o.escalation_multiplier).collect();
    let esc_mean = esc_values.iter().sum::<f64>() / esc_values.len() as f64;
    let esc_variance = esc_values.iter().map(|e| (e - esc_mean).powi(2)).sum::<f64>() / esc_values.len() as f64;

    let energy_creep = drift > 0.15;
    let emotional_flatness = esc_variance < 0.5;

    let risk: f64 = if energy_creep && emotional_flatness { 0.6 }
        else if energy_creep { 0.4 }
        else if emotional_flatness { 0.3 }
        else { 0.1 };

    let score = ((1.0 - risk) * 100.0_f64).clamp(0.0, 100.0);

    DomainAnalysis {
        domain: AilDomain::SessionDrift, score, risk,
        details: format!("drift={:.4} q1={:.3} q4={:.3} esc_var={:.3} creep={} flat={}",
...
fn report_to_json(report: &AilReport) -> Result<String, String> {
    ...
    write!(json, "\"overall_status\":\"{}\",", report.score.status.name()).map_err(|e| e.to_string())?;
    write!(json, "\"recommendation_count\":{},", report.recommendations.len()).map_err(|e| e.to_string())?;
    ...
    for (i, da) in report.domain_analyses.iter().enumerate() {
        ...
        write!(json, "{{\"domain\":\"{}\",\"score\":{:.1},\"risk\":{:.4},\"details\":\"{}\"}}",
            da.domain.name(), da.score, da.risk, da.details.replace('\"', "\\\"")).map_err(|e| e.to_string())?;
    }
    write!(json, "],").map_err(|e| e.to_string())?;

    // Fatigue
    write!(json, "\"fatigue_analysis\":{{").map_err(|e| e.to_string())?;
    write!(json, "\"fatigue_score\":{:.2},", report.fatigue.fatigue_score).map_err(|e| e.to_string())?;
    write!(json, "\"peak_frequency\":{:.4},", report.fatigue.peak_frequency).map_err(|e| e.to_string())?;
    write!(json, "\"harmonic_density\":{:.4},", report.fatigue.harmonic_density).map_err(|e| e.to_string())?;
    write!(json, "\"temporal_density\":{:.4},", report.fatigue.temporal_density).map_err(|e| e.to_string())?;
    write!(json, "\"recovery_factor\":{:.4},", report.fatigue.recovery_factor).map_err(|e| e.to_string())?;
    write!(json, "\"risk_level\":\"{}\"", report.fatigue.risk_level).map_err(|e| e.to_string())?;
    write!(json, "}},").map_err(|e| e.to_string())?;

    // Voice efficiency
    write!(json, "\"voice_efficiency\":{{").map_err(|e| e.to_string())?;
    write!(json, "\"avg_voices\":{:.1},", report.voice_efficiency.avg_voices).map_err(|e| e.to_string())?;
    write!(json, "\"peak_voices\":{},", report.voice_efficiency.peak_voices).map_err(|e| e.to_string())?;
    write!(json, "\"budget_cap\":{},", report.voice_efficiency.budget_cap).map_err(|e| e.to_string())?;
    write!(json, "\"utilization_pct\":{:.1},", report.voice_efficiency.utilization_pct).map_err(|e| e.to_string())?;
    write!(json, "\"efficiency_score\":{:.1}", report.voice_efficiency.efficiency_score).map_err(|e| e.to_string())?;
    write!(json, "}},").map_err(|e| e.to_string())?;

    // Spectral clarity
    write!(json, "\"spectral_clarity\":{{").map_err(|e| e.to_string())?;
    write!(json, "\"sci_advanced\":{:.4},", report.spectral_clarity.sci_advanced).map_err(|e| e.to_string())?;
    write!(json, "\"sci_limit\":{:.2},", report.spectral_clarity.sci_limit).map_err(|e| e.to_string())?;
    write!(json, "\"clarity_score\":{:.1},", report.spectral_clarity.clarity_score).map_err(|e| e.to_string())?;
    write!(json, "\"overlap_count\":{}", report.spectral_clarity.overlap_count).map_err(|e| e.to_string())?;
    write!(json, "}},").map_err(|e| e.to_string())?;

    // Volatility alignment
    write!(json, "\"volatility_alignment\":{{").map_err(|e| e.to_string())?;
    write!(json, "\"alignment_score\":{:.1},", report.volatility_alignment.alignment_score).map_err(|e| e.to_string())?;
    write!(json, "\"energy_variance\":{:.4},", report.volatility_alignment.energy_variance).map_err(|e| e.to_string())?;
    write!(json, "\"escalation_range\":{:.2}", report.volatility_alignment.escalation_range).map_err(|e| e.to_string())?;
    write!(json, "}},").map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn test_analyze_full() {
        ...
        for da in &report.domain_analyses {
            assert!(da.score >= 0.0 && da.score <= 100.0, "Domain {:?} score={}", da.domain, da.score);
            assert!(da.risk >= 0.0 && da.risk <= 1.0, "Domain {:?} risk={}", da.domain, da.risk);
        }
    }
    ...
    fn test_ail_score_formula() {
        ...
        assert!((report.score.value - expected).abs() < 0.01,
            "Expected score={:.2}, got={:.2}", expected, report.score.value);
    }
    ...
    fn test_recommendations_sorted_by_impact() {
        ...
        for i in 1..report.recommendations.len() {
            assert!(report.recommendations[i-1].impact_score >= report.recommendations[i].impact_score,
                "Recommendations not sorted by impact at index {}", i);
        }
    }
    ...
    fn test_with_pbse_result() {
        let pbse_result = PbseResult {
            domains: (0..10).map(|i| crate::qa::pbse::DomainResult {
    ...
    fn test_gen_analysis_steps() {
        ...
        let low_wins = steps.iter().filter(|s| s.win_multiplier > 0.0 && s.win_multiplier <= 10.0).count();
        ...
        assert!(jackpots > 0 || high_wins > 0, "Should have high-intensity events");
    ...
    fn test_voice_efficiency_bounds() {
        ...
        assert!(report.voice_efficiency.efficiency_score >= 0.0 && report.voice_efficiency.efficiency_score <= 100.0);
    }
    ...
    fn test_spectral_clarity_bounds() {
        ...
        assert!(report.spectral_clarity.clarity_score >= 0.0 && report.spectral_clarity.clarity_score <= 100.0);
    }
}
