...
| 3 | 🔴 P0 | `helix_screen.dart` cela | 10500 LOC monolit — 13 dock panela u JEDNOM fajlu → kompilacija sporija, refactor opasnost | XL | ✅ 2026-05-07 (8a2e91cf) — Partial extraction kroz Dart `part of` mehanizam: `helix_omnibar_atoms.dart` (174 LOC: _OmniPill/_OmniIconBtn/_ModeBadge/_TransportBtn), `helix_dock_widgets.dart` (210 LOC: _DockTab/_DockCard/_DockLabel), `helix_minimode_widgets.dart` (71 LOC: _MiniModeSection/_MiniDivider/_ComplianceDot). 455 LOC ekstraktovano + 2374 LOC iz `engine_connected_layout.dart` + 580 LOC iz `slotlab_lower_zone_widget.dart`. **3244/3244 testova zelena**, 0 API breakage (private name access preserved kroz `part`). |
| 4 | 🟡 P1 | `helix_screen.dart:1517` | Anticipation glow magic number `* 0.6 + 60` — pretpostavlja centrirani 60% grid; pomera se kad se PremiumSlotPreview pomeri | M | ✅ 2026-05-07 — Magic literali ekstraktovani u file-private named konstante (`_kSlotGridWidthRatio = 0.6`, `_kSlotGridLeftOffsetPx = 60.0`, `_kSlotGridVInsetPx = 60.0`) sa eksplicitnim docstring-om koji objašnjava layout assumption. Dodat `TODO(URP-future)` za real fix kroz `GlobalKey` lookup live PremiumSlotPreview RenderBox-a. Layout izmena sad jednomeno-edit. |
| 5 | 🟡 P1 | `helix_screen.dart:1741` | ARCHITECT mode override-uje user `_dockHeight` na `0.5*screenH` — gubi customizaciju bez upozorenja | S | ✅ 2026-05-07 — Razdvojeno na 2 polja: `_dockHeightCompose` (initial 380) i `_dockHeightArchitect` (sentinel `-1` → lazy init na 0.5×screenH na prvi build, posle drag-able). `_resolveDockHeight()` helper bira po modu. Drag handle update-uje **active mode** height (ne više sve), ARCHITECT clamp do `0.9 × screenH` (analyst mode), COMPOSE ostaje 180-600. Mode toggle više ne briše custom resize. |
