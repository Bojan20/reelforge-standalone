...
// Atomic helper widget classes extracted to keep this file navigable.
// All `_` private classes remain in the same Dart library scope.
part 'engine_layout/engine_layout_widgets.dart';
...
class _EngineConnectedLayoutState extends State<EngineConnectedLayout>
  ...
  void _handleNewProject(EngineProvider engine) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('New Project', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textPrimary)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('Create a new project?', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textSecondary)),
  ...
  void _showProjectSettingsDialog(EngineProvider engine) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: FluxForgeTheme.bgElevated,
        title: Text('Project Settings', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textPrimary)),
        content: SizedBox(
          width: 400,
          child: Column(
            mainAxisSize: MainAxisSize.min,
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: Text('Close', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.accentBlue)),
          ),
        ],
      ),
    );
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: issues.isEmpty
              ? [Text('No issues found.', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textSecondary))]
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: Text('OK', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.accentBlue)),
          ),
        ],
      ),
    );
                  ...
                  child: DropdownButton<String>(
                    ...
                    items: ['spin', 'win', 'bigWin', 'feature', 'bonus', 'ui', 'ambient', 'music'].map((cat) {
                      return DropdownMenuItem(
                        value: cat,
                        child: Text(cat.toUpperCase(), style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textPrimary)),
                      );
                    }).toList(),
                    onChanged: (val) {
  ...
  void _showSettingsDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: FluxForgeTheme.bgElevated,
        title: Text('Middleware Settings', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textPrimary)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Events: ${_middlewareEvents.length}', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textSecondary)),
            const SizedBox(height: 8),
      ...
      child: Actions(
        ...
        children: [
          MainLayout(
            ...
            onChannelWidthChange: (channelId, width) {
              final mixerProvider = context.read<MixerProvider>();
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('Select an event to view timeline', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textSecondary)),
          ],
        ),
      );
    }
  ...
  Widget _buildLayersAsActionsTable(double availableWidth, SlotCompositeEvent? event, MiddlewareProvider provider) {
    if (event == null) {
      return Container(
        ...
        child: Center(
          child: Text('No event selected', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textSecondary)),
        ),
      );
    }
        ...
        children: [
          ...
          if (event.layers.isEmpty)
            Container(
              ...
              child: Center(
                child: Text('No layers. Click "Add" to create one.', style: FluxForgeTheme.dockSans(color: FluxForgeTheme.textTertiary)),
              ),
            )
          else
  ...
  ChannelStripData? _getSelectedChannelData() {
    ...
    // MixerChannel IDs use 'ch_' prefix (e.g. 'ch_0'), but TimelineTrack IDs don't (e.g. '0')
    final trackSearchId = _selectedTrackId!.startsWith('ch_')
        ? _selectedTrackId!.replaceFirst('ch_', '')
        : _selectedTrackId;
    final track = _tracks.cast<timeline.TimelineTrack?>().firstWhere(
      (t) => t?.id == trackSearchId,
      orElse: () => null,
    );
  ...
  /// Convert MixerProvider buses to UltimateMixerChannel list (dynamic only)
  ...
  Widget _buildMixerScreenView(dynamic metering) {
    ...
    // Build peakReader closures — 120fps direct FFI reads, bypasses widget rebuild pipeline
    final peakReaders = <String, (double, double) Function()>{};
    for (final ch in mixerProvider.channels) {
      final trackId = ch.id.startsWith('ch_') ? ch.id.substring(3) : ch.id;
      final trackIdInt = int.tryParse(trackId) ?? 0;
      final engineTrackId = ch.trackIndex ?? trackIdInt;
    ...
    for (final aux in mixerProvider.auxes) {
      ...
      if (idx >= 0) {
        peakReaders[aux.id] = () => NativeFFI.instance.getBusPeak(idx);
      }
    }
    return MixerScreen(
  ...
  /// Toggle detached floating mixer window
  void _toggleDetachedMixer() {
    if (FloatingMixerRegistry.instance.isOpen) {
      FloatingMixerRegistry.instance.close();
      return;
    }
    // Switch to edit view if currently in inline mixer
    if (_appViewMode == AppViewMode.mixer) {
      setState(() => _appViewMode = AppViewMode.edit);
    }
    setState(() => _isMixerDetached = true);
    // Show floating mixer after frame so context is valid
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      FloatingMixerWindow.show(
        context: context,
        viewController: _mixerViewController,
        callbacks: _buildMixerCallbacks(),
        onClosed: () {
          if (mounted) {
            setState(() => _isMixerDetached = false);
          }
        },
      );
    });
  ...
  /// Build MixerCallbacks bundle for floating mixer window
  MixerCallbacks _buildMixerCallbacks() {
    final mixerProvider = context.read<MixerProvider>();
    final engine = EngineApi.instance;
    return MixerCallbacks(
      buildChannels: () {
        final mp = context.read<MixerProvider>();
        final channels = <ultimate.UltimateMixerChannel>[];
        for (final ch in mp.channels) {
          ...
          channels.add(ultimate.UltimateMixerChannel(
            id: ch.id, name: ch.name, type: ultimate.ChannelType.audio,
            color: ch.color, volume: ch.volume, pan: ch.pan,
            panRight: ch.panRight, isStereo: ch.isStereo,
      ...
      buildBuses: () => _buildBusChannels(context.read<MixerProvider>()),
      buildAuxes: () => _buildAuxChannels(context.read<MixerProvider>()),
      buildVcas: () => _buildVcaChannels(context.read<MixerProvider>()),
      buildMaster: () {
      ...
      onChannelSelect: (id) {
        setState(() => _selectedTrackId = id == 'master' ? '0' : id);
      },
      onVolumeChange: (id, vol) {
        if (_isBusId(id)) {
          _onBusVolumeChange(id, vol);
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(),
            child: Text('Cancel',
                style: FluxForgeTheme.dockSans(color: const Color(0xFF888899))),
          ),
          TextButton(
            onPressed: () {
  ...
  Widget _buildUltimateMixerContent(dynamic metering, bool isPlaying) {
    ...
    // Build peakReader closures — 120fps direct FFI reads
    final peakReaders2 = <String, (double, double) Function()>{};
    for (final ch in mixerProvider.channels) {
      final trackId = ch.id.startsWith('ch_') ? ch.id.substring(3) : ch.id;
      final trackIdInt = int.tryParse(trackId) ?? 0;
      final engineTrackId = ch.trackIndex ?? trackIdInt;
    ...
    for (final aux in mixerProvider.auxes) {
      final idx = MixerProvider.busIdToEngineBusIndex(aux.outputBus ?? 'aux');
      if (idx >= 0) {
        peakReaders2[aux.id] = () => NativeFFI.instance.getBusPeak(idx);
      }
    }
  ...
  Widget _buildMiddlewareMixerContent(dynamic metering, bool isPlaying) {
    ...
    // Build peakReader closures for middleware buses — 120fps direct FFI reads
    final mwPeakReaders = <String, (double, double) Function()>{};
    for (final (name, _, _) in middlewareBuses) {
      final busId = name.toLowerCase();
      final idx = _busIdToEngineBusIndex(busId);
      if (idx >= 0) {
        mwPeakReaders[busId] = () => NativeFFI.instance.getBusPeak(idx);
      }
    }
  ...
  // Bus state — only master is pre-initialized; DAW buses are dynamic via MixerProvider
  final Map<String, double> _busVolumes = {'master': 1.0};
  ...
  /// Direct master peak read — 120fps, bypasses widget rebuild pipeline
  (double, double) _readMasterPeak() {
    final m = SharedMeterReader.instance;
    if (m.hasChanged) {
      final snap = m.readMeters();
      return (snap.masterPeakL, snap.masterPeakR);
    }
    return NativeFFI.instance.getBusPeak(0); // fallback: master bus 0
  }
  ...
  bool _isBusId(String id) {
    if (id == 'master' || id.startsWith('bus_')) return true;
    // Middleware bus IDs (used in middleware/slotlab mixer views)
    const middlewareBusIds = {'sfx', 'music', 'voice', 'amb', 'ui', 'ambience', 'reels', 'wins', 'vo'};
    return middlewareBusIds.contains(id);
  }
  ...
  int _busIdToEngineBusIndex(String busId) {
    ...
    if (busMap.containsKey(busId)) return busMap[busId]!;
    if (busId.startsWith('bus_')) return 0; // User-created buses → Master
    return -1;
  }
  ...
  void _onBusPanChange(String busId, double pan) {
    // DAW buses and master — delegate to MixerProvider
    if (busId.startsWith('bus_') || busId == 'master') {
      context.read<MixerProvider>().setChannelPan(busId, pan);
      return;
    }
    // Middleware buses — local state + FFI
  ...
  void _onBusPanRightChange(String busId, double pan) {
    // DAW buses and master — delegate to MixerProvider
    if (busId.startsWith('bus_') || busId == 'master') {
      context.read<MixerProvider>().setChannelPanRight(busId, pan);
      return;
    }
    // Middleware buses — local state + FFI
    final clampedPan = pan.clamp(-1.0, 1.0);
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text('${_bpmMinRange.toStringAsFixed(0)}', style: FluxForgeTheme.dockMono(color: FluxForgeTheme.textSecondary, size: 10)),
                    ],
                  ),
                  Row(
                    children: [
                      ...
                      Text('${_bpmMaxRange.toStringAsFixed(0)}', style: FluxForgeTheme.dockMono(color: FluxForgeTheme.textSecondary, size: 10)),
                    ],
                  ),
...
// ─────────────────────────────────────────────────────────────────────────────
// ATOMIC HELPER WIDGETS → engine_layout/engine_layout_widgets.dart (part file)
// ─────────────────────────────────────────────────────────────────────────────
// _MeterRow, _StatusIndicator, _InspectorField*, _ToolbarButton*,
// _MiniDropdown, _MiniInput, _MiniSlider, _MiniToggle,
// _CommandGroup, _CommandChip, _CommandIconBtn, _CommandToggle,
// _ParamBox, _LayerToggle, _Cell*, _SettingsRow, _InsertMenuOption,
// _AudioEditorDialog, _MiddlewareTimePainter*, _SendDialogResult, etc.
