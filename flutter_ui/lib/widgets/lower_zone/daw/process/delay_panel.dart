/// DAW Delay Panel
///
/// FF-DLY delay wrapper with bypass/wet-dry controls.
...
class _DelayPanelState extends State<DelayPanel> {
  ...
  void _ensureProcessorLoaded() {
    ...
    final node = chain.nodes.where((n) => n.type == DspNodeType.delay).firstOrNull;
    if (node != null) {
      _nodeId = node.id;
    } else {
      DspChainProvider.instance.addNode(trackId, DspNodeType.delay);
      final updated = DspChainProvider.instance.getChain(trackId);
      _nodeId = updated.nodes.where((n) => n.type == DspNodeType.delay).firstOrNull?.id;
    }
    _autoLoaded = true;
  }
  ...
  DspNode? get _node {
    ...
    return _nodeId != null
        ...
        : chain.nodes.where((n) => n.type == DspNodeType.delay).firstOrNull;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        Expanded(child: FabFilterDelayPanel(trackId: trackId)),
      ],
    );
  }
