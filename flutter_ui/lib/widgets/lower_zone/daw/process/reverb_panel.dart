/// DAW Reverb Panel
///
/// FF-R reverb wrapper with bypass/wet-dry controls.
...
class _ReverbPanelState extends State<ReverbPanel> {
  ...
  void _ensureProcessorLoaded() {
    ...
    final node = chain.nodes.where((n) => n.type == DspNodeType.reverb).firstOrNull;
    if (node != null) {
      _nodeId = node.id;
    } else {
      ...
      _nodeId = updated.nodes.where((n) => n.type == DspNodeType.reverb).firstOrNull?.id;
    }
    _autoLoaded = true;
  }
  ...
  DspNode? get _node {
    ...
    return _nodeId != null
        ...
        : chain.nodes.where((n) => n.type == DspNodeType.reverb).firstOrNull;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        // FF Panel
        Expanded(
          child: FabFilterReverbPanel(trackId: trackId),
        ),
      ],
    );
  }
