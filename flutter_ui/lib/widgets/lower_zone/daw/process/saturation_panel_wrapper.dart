...
///
/// FF-SAT saturation wrapper with bypass/wet-dry controls.
...
class _SaturationPanelWrapperState extends State<SaturationPanelWrapper> {
  ...
  void _ensureProcessorLoaded() {
    ...
    final node = chain.nodes.where((n) => n.type == DspNodeType.saturation).firstOrNull;
    if (node != null) {
      _nodeId = node.id;
    } else {
      DspChainProvider.instance.addNode(trackId, DspNodeType.saturation);
      final updated = DspChainProvider.instance.getChain(trackId);
      _nodeId = updated.nodes.where((n) => n.type == DspNodeType.saturation).firstOrNull?.id;
    }
    _autoLoaded = true;
  }
  ...
  DspNode? get _node {
    ...
    return _nodeId != null
        ...
        : chain.nodes.where((n) => n.type == DspNodeType.saturation).firstOrNull;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        Expanded(child: FabFilterSaturationPanel(trackId: trackId)),
      ],
    );
  }
