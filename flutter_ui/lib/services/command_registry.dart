...
class CommandRegistry {
  ...
  void registerDawCommands({
    ...
    VoidCallback? onShowRazorEdit,
    VoidCallback? onShowMixSnapshots,
    VoidCallback? onShowMetadataBrowser,
    VoidCallback? onShowScreensets,
    VoidCallback? onShowProjectTabs,
    ...
    VoidCallback? onDawTabMix,
    VoidCallback? onDawTabProcess,
    VoidCallback? onDawTabDeliver,
  }) {
    registerAll([
      // ─── FILE ─────────────────────────────────────────────────────────────
      PaletteCommand(
      ...
      PaletteCommand(
        id: 'tools.razor_edit',
        label: 'Razor Edit',
        description: 'Alt+drag multi-track range selection with merged processing',
        category: PaletteCategory.tools,
        icon: Icons.carpenter,
        keywords: ['razor', 'edit', 'range', 'selection', 'multi', 'track'],
        onExecute: onShowRazorEdit,
      ),
      PaletteCommand(
        id: 'tools.mix_snapshots',
        label: 'Mix Snapshots',
        description: 'Save/recall mixer state with selective capture',
        category: PaletteCategory.tools,
        icon: Icons.camera,
        keywords: ['mix', 'snapshot', 'recall', 'save', 'state', 'mixer'],
        onExecute: onShowMixSnapshots,
      ),
      PaletteCommand(
        id: 'tools.metadata_browser',
        label: 'Metadata Browser',
        description: 'BWF/iXML/ID3v2/RIFF metadata parsing with boolean search',
        category: PaletteCategory.tools,
        icon: Icons.info_outline,
        keywords: ['metadata', 'browser', 'bwf', 'ixml', 'id3', 'riff', 'tag'],
        onExecute: onShowMetadataBrowser,
      ),
      PaletteCommand(
        id: 'tools.screensets',
        label: 'Screensets',
        description: '10 numbered UI state slots with save/recall',
        category: PaletteCategory.tools,
        icon: Icons.window,
        keywords: ['screenset', 'layout', 'window', 'save', 'recall', 'slot'],
        onExecute: onShowScreensets,
      ),
      PaletteCommand(
        id: 'tools.project_tabs',
        label: 'Project Tabs',
        description: 'Multi-project tab system with swap-in/swap-out',
        category: PaletteCategory.tools,
        icon: Icons.tab,
        keywords: ['project', 'tab', 'multi', 'switch', 'swap'],
        onExecute: onShowProjectTabs,
      ),
      PaletteCommand(
        id: 'tools.sub_projects',
        label: 'Sub-Projects',
        description: 'Nested .rfproj references on timeline with proxy render',
        category: PaletteCategory.tools,
        icon: Icons.account_tree,
        keywords: ['sub', 'project', 'nested', 'reference', 'proxy'],
        onExecute: onShowSubProjects,
      ),

      // ─── DAW TAB NAVIGATION ────────────────────────────────────────────────
      PaletteCommand(
        id: 'daw.tab.browse',
        label: 'Switch to Browse Tab',
        description: 'Show DAW Browse panel (Files, Presets, Plugins)',
        category: PaletteCategory.navigate,
        icon: Icons.folder_outlined,
        keywords: ['browse', 'tab', 'files', 'presets', 'plugins'],
        onExecute: onDawTabBrowse,
      ),
      PaletteCommand(
        id: 'daw.tab.edit',
        label: 'Switch to Edit Tab',
        description: 'Show DAW Edit panel (Timeline, Piano Roll, Tools)',
        category: PaletteCategory.navigate,
        icon: Icons.edit_outlined,
        keywords: ['edit', 'tab', 'timeline', 'piano', 'tools'],
        onExecute: onDawTabEdit,
      ),
      PaletteCommand(
        id: 'daw.tab.mix',
        label: 'Switch to Mix Tab',
        description: 'Show DAW Mix panel (Mixer, Sends, Pan, Automation)',
        category: PaletteCategory.navigate,
        icon: Icons.tune,
        keywords: ['mix', 'tab', 'mixer', 'fader', 'console'],
        onExecute: onDawTabMix,
      ),
      PaletteCommand(
        id: 'daw.tab.process',
        label: 'Switch to Process Tab',
        description: 'Show DAW Process panel (EQ, Comp, FX Chain)',
        category: PaletteCategory.navigate,
        icon: Icons.equalizer,
        keywords: ['process', 'tab', 'eq', 'compressor', 'fx', 'dsp'],
        onExecute: onDawTabProcess,
      ),
      PaletteCommand(
        id: 'daw.tab.deliver',
        label: 'Switch to Deliver Tab',
        description: 'Show DAW Deliver panel (Export, Stems, Bounce)',
        category: PaletteCategory.navigate,
        icon: Icons.upload_file,
        keywords: ['deliver', 'tab', 'export', 'bounce', 'render', 'stems'],
        onExecute: onDawTabDeliver,
      ),
    ]);
  }
}
